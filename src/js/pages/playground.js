...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function updatePlaygroundCode(featureTitle, htmlMarkup, configCode) {
    ...
    if (playgroundName) playgroundName.innerText = featureTitle;
    ...
    if (playgroundSnippet) playgroundSnippet.innerText = codeText;
  }

  if (btnCopyPlayground && playgroundSnippet) {
  ...
  const heroTodayDate = document.getElementById('hero-today-date');
  if (window.NDPUtils && heroTodayDate) {
    ...
    heroTodayDate.innerText = formattedBs + ' (' + formattedAd + ')';
  }

  // 2. Hero Interactive Inline Picker
  if (window.NepaliDatePicker && document.getElementById('hero-inline-picker')) {
  ...
  if (singleInput && window.NepaliDatePicker) {
    new NepaliDatePicker(singleInput, {
      ...
      onChange: function(date) {
        if (singleOutput) singleOutput.innerHTML = '<strong>Selected Date details:</strong><br>' + JSON.stringify(date, null, 2);
      }
    });
  }
  ...
  if (multiInput && window.NepaliDatePicker) {
    new NepaliDatePicker(multiInput, {
      ...
      onChange: function(dates) {
        if (multiOutput) multiOutput.innerHTML = '<strong>Selected Dates Array:</strong><br>' + JSON.stringify(dates, null, 2);
      }
    });
  }
  ...
  if (window.NepaliDatePicker && document.getElementById('inline-picker-container')) {
    new NepaliDatePicker('#inline-picker-container', {
      inline: true,
      theme: 'midnight',
      lang: 'ne',
      onChange: function(date) {
        if (inlineOutput) inlineOutput.innerText = 'Selected: ' + date.formatted;
      }
    });
  }
  ...
  if (timeInput && window.NepaliDatePicker) {
    new NepaliDatePicker(timeInput, {
      ...
      enableTime: true,
      onChange: function(date) {
        if (timeOutput) timeOutput.innerHTML = '<strong>Date & Time details:</strong><br>' + JSON.stringify(date, null, 2);
        triggerDemo2Code();
      }
    });
  ...
  if (maskInput && window.NepaliDatePicker) {
    new NepaliDatePicker(maskInput, {
      ...
      onChange: function(date) {
        if (maskOutput) maskOutput.innerHTML = '<strong>Typed Valid Date:</strong> ' + date.formatted;
        triggerDemo4Code();
      }
    });
  ...
  if (window.NepaliDatePicker && document.getElementById('custom-renderer-calendar-container')) {
  ...
  if (window.NepaliDatePicker && document.getElementById('large-inline-container')) {
    new NepaliDatePicker('#large-inline-container', {
      ...
      onChange: function(dates) {
        if (dates && dates.length > 0) {
          ...
          if (largeInlineOutput) largeInlineOutput.innerHTML = `<strong>Selected Dates (${dates.length}):</strong><br>${formatted}`;
        } else {
          if (largeInlineOutput) largeInlineOutput.innerHTML = 'Selected Dates: None';
        }
        triggerDemo10Code();
      }
    });
  ...
  if (bookingInput && window.NepaliDatePicker) {
    new NepaliDatePicker(bookingInput, {
      ...
      onChange: function(date) {
        if (!date) {
          if (slotsContainer) slotsContainer.classList.add('ndp-hidden');
          if (bookingCard) bookingCard.querySelector('h4').textContent = 'Select a date to view available booking slots.';
        ...
        if (bookingCard) bookingCard.querySelector('h4').textContent = '🗓 Date Selected';
        if (bookedDateLbl) bookedDateLbl.textContent = `${date.year}-${date.month}-${date.day}`;
        if (slotsGrid) slotsGrid.innerHTML = '';
        
        if (booking.times.length === 0) {
          if (slotsGrid) slotsGrid.innerHTML = '<span style="color: #ef4444; font-size: 0.85rem;">No available slots. Fully Booked!</span>';
          if (slotsContainer) slotsContainer.classList.remove('ndp-hidden');
          return;
        }
  ...
  // STRICT API DEMO INITIALIZATION
  if (window.NepaliDatePicker) {
    ...
    // Selection Modes, Unicode & English Language
    new NepaliDatePicker('#range-strict-picker-new', { theme: 'classic-light', lang: 'en', range: true });
    new NepaliDatePicker('#multiple-strict-picker-new', { theme: 'classic-light', lang: 'en', multiple: true });
    new NepaliDatePicker('#unicode-strict-picker-new', { theme: 'classic-light', lang: 'en', unicodeDate: true });
    new NepaliDatePicker('#english-strict-picker-new', { theme: 'classic-light', language: 'english' });

    // Category text configurations dynamically mapped to labels, headers, and placeholders
      ...
      function validateDates() {
        ...
        if (startVal && endVal) {
          ...
          if (comp < 0) {
            ...
            alertTextEl.innerText = serviceConfigs[catSelect.value].alertMsg;
            endPicker.clear();
  ...
  // Theme Gallery Card Auto-Initializers & Code Drawer
  ...
  document.querySelectorAll('.theme-card').forEach(card => {
    ...
    card.addEventListener('click', (e) => {
      ...
      if (themeCodeDrawer) themeCodeDrawer.classList.remove('hidden');
      if (themeCodeName) themeCodeName.innerText = themeName;
      ...
      if (themeCodeSnippet) themeCodeSnippet.innerText = codeText;
      if (themeCodeDrawer) themeCodeDrawer.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
    });
  });

  if (btnCopyThemeCode) {
    btnCopyThemeCode.addEventListener('click', () => {
      const text = themeCodeSnippet ? themeCodeSnippet.innerText : '';
      navigator.clipboard.writeText(text).then(() => {
